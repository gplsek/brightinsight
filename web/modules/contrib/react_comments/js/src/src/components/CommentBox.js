...
class CommentBox extends Component {
  ...
  userCanPost = () => {
    ...
    if (isAnon && !this.state.anonName) {
      ...
      this.setState({ message: { message: 'Please provide your email to post as a guest', type: 'error'}});
      return false;
    }
    else if (isAnon && this.state.anonEmail && !validateEmail(this.state.anonEmail)) {
      this.setState({ message: { message: 'Please provide a valid email address', type: 'error'}});
      return false;
    }
    else {
      return true;
          ...
          <div className="rc_throbber-wrapper">
            ...
            <div className="rc_input-wrapper">
              ...
              <div className="rc_input-actions">
                ...
                { (type === 'reply' && user && userCanPostComments) && <button onClick={() => this.postReply(this.getCommentText()) } className="rc_add-comment">Post{user.name && ` as ${user.name}`}</button> }
                { (type === 'comment' && user && userCanPostComments) && <button onClick={() => this.postComment(this.getCommentText())} className="rc_add-comment">Post{user.name && ` as ${user.name}`}</button> }
              </div>
            </div>
          </div>
              ...
              <div>
                ...
                <div className="rc_anon-form-input-wrapper">
                  ...
                  { (settings.anonymous !== Constants.anonMayNotContact) &&
                    <input onChange={this.handleAnonFormChange} id="rc_email" type="text" placeholder="Email"/>
                  }
                </div>
              </div>
            </div> }
